...
/**
* description: 从特定网站中筛选出相应的pdf页面，下载到本地，并更新数据库
* note: 异常造成的线程中断的问题
...
public class DownloadPdf{
	 ...
	 * @throws 
	 */
	public DownloadPdf(RankUrl url,LinkFilter filter,Lock lock){
		myUrl=url;
		myFilter=filter;
		MyCrawler.count++;
		this.lock=lock;
	...
	public void run()
	{
		...
		try {
			...
			Set<String> links;
			if(contType.contains("GB2312"))
				links=HtmlParserTool.extracLinks_gb(myUrl.getUrl(),myFilter);
			else
				links=HtmlParserTool.extracLinks(myUrl.getUrl(),myFilter);
			//得到未访问的 URL 
			...
			for(String link:links)	
			{
				if(link == null || !link.endsWith(".pdf"))
					continue;
				String _link = new String(link);
				link = "";
				for(int i = 0;i < _link.length();++i){
					char _c = _link.charAt(i);
					if(_c == ' ')
						link += "%20";
					else
						link += _c;
	...
	public void runForBerkeley()
	{
		...
		try {
			...
			Set<String> father_links=HtmlParserTool.extracLinks(myUrl.getUrl(),myFilter);
			//得到未访问的 URL 
			
			for(String father_link:father_links){
				if(!father_link.startsWith(myUrl.url))
					continue;
				RankUrl father_myUrl = new RankUrl(father_link);
				Set<String> links=HtmlParserTool.extracLinks(father_myUrl.getUrl(),myFilter);
				...
				System.out.println("finish for son:"+father_link);
			}
		} catch (HttpException e) {
			// 发生致命的异常，可能是协议不对或者返回的内容有问题
			e.printStackTrace();
		...
		System.out.println("finish for father:"+myUrl.url);
	}
