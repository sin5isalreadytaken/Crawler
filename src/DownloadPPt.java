...
/**
* description: 从特定网站中筛选出相应的ppt页面，下载到本地，并更新数据库
* note: 异常造成的线程中断的问题
...
public class DownloadPPt{
	 ...
	 * @throws 
	 */
	public DownloadPPt(RankUrl url,LinkFilter filter,Lock lock){
		myUrl=url;
		myFilter=filter;
		MyCrawler.count++;
		this.lock=lock;
	...
	public void run()
	{
		...
		try {
			...
			{
				if(!link.endsWith(".ppt") && !link.endsWith(".pptx"))
					continue;
				...
				if(link.endsWith(".ppt"))
					filePath = oripath + "\\"+ C_ID+".ppt";
				else
					filePath = oripath + "\\"+ C_ID+".pptx";
	...
	public void DataBase(String IntoDataBase,String FilePath) throws SQLException
	{
		...
		System.out.printf("DownloadPPt:%s\n", sql);
